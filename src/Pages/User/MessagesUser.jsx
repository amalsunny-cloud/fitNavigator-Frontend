...
const MessagesUser = () => {
  ...
  const fetchReceivedMessages = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:3000/fetch-all-user-messages',{
        headers: {
  ...
  const fetchSentMessages = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:3000/user-sent-messages/${userId}`);
  ...
  const userMessageToTrainer = async(userId, messageText)=>{
    try{
      ...
      const trainerResponse = await axios.get(`http://localhost:3000/user-messages/${userId}`)
    ...
    const response = await axios.post(`http://localhost:3000/user-messages`, newMessage, {
  ...
  const handleDeleteMessage = async (id) => {
    try {
      ...
      const response = await axios.delete(`http://localhost:3000/delete-usermessages/${id}`);
